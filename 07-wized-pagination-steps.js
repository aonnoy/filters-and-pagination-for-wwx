...
window.Wized.push(async (Wized) => {
  ...
  const activeClass =
    wrapperElement.getAttribute("wized-pagination-active-class") || "active";
  ...
  // Modified executeWizedRequest function with check
  const executeWizedRequest = debounce(async () => {
    const currentPage = Wized.data.v[currentPageVariableName];
    const totalPages = Wized.data.v[totalPagesVariableName];
  ...
  const resetCurrentPageAndExecuteWizedRequest = () => {
    ...
    if (
      ...
      window.executeWizedRequest(); // Call the centralized function
  ...
  const createPageNumber = (i, currentPage) => {
    let isActive = i === currentPage ? activeClass : "";
    return `<a wized-pagination-element="number" href="#" class="link-block w-inline-block ${isActive}"><div>${i}</div></a>`;
  };
  ...
  // Function to execute the Wized request directly
  const executeWizedRequestDirectly = async () => {
    if (
      wizedRequestName &&
      Wized.requests &&
      typeof Wized.requests.execute === "function"
  ...
  const updatePagination = (page, totalPages) => {
    ...
    if (Wized.data.v[currentPageVariableName] !== currentPage) {
      ...
      // Execute the Wized request directly for pagination change
      executeWizedRequestDirectly();
    }
  ...
  // Initialize pagination and start checking for page changes
  const setupPagination = async () => {
    ...
    attachEventListeners(lastKnownTotalPages);
  };
